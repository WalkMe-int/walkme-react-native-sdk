...
# one integration gap that can only be fixed from the consuming app's Podfile
# `post_install`: CocoaPods links an SPM product to the Pods target but never
# embeds its framework into the app bundle. Rather than have every app copy/paste
# that logic, the bridge ships it here and the app just calls
