# Legacy — WalkMe React Native example

React Native **0.81.4** on the **Legacy Architecture**. The bridge resolves
through `NativeModules`; the app header prints the architecture the JavaScript
runtime actually landed on, so you can confirm it.

| | |
|---|---|
| React Native | 0.81.4 |
| Architecture | Legacy — `newArchEnabled=false` in `android/gradle.properties` |
| WalkMe flavor | `WalkMeEditor` (Power Mode) — `walkme.walkmeMode` in `package.json` |
| Android package | `com.walkmeexamplelegacy` |
| iOS scheme | `WalkMeExampleLegacy` |

The New Architecture counterpart is [`../new-arch`](../new-arch). Both apps share
the same `App.tsx`, which is the point: nothing in it is architecture-aware.

React Native removed the Legacy Architecture in 0.82, so this app also pins the
oldest configuration the bridge is regularly built against.
...
Metro can never load the SDK's dev copy of React Native — 0.85 here — next to
this app's 0.81.
...
## iOS: the fmt workaround

React Native ≤ 0.81 vendors fmt 11.0.2, which the Clang in Xcode 16.3+ rejects
under C++20. This app's `Podfile` builds the `fmt` pod as C++17 in `post_install`
to work around it. That has nothing to do with the WalkMe bridge — fmt fixed it
in 11.1 and React Native picked it up in 0.82.
